...
function isForbiddenProtocolNodeImport(specifier: string, ownPackageName?: string): boolean { if (specifier === ownPackageName) return false; if (specifier === "@earendil-works/pi-coding-agent") return false; if (specifier.startsWith("@kybernetria/pi-protocol-")) return false; return /^(pi-[a-z0-9-]+|@[a-z0-9_-]+\/pi-[a-z0-9-]+)$/.test(specifier); }
