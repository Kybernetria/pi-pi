...
export function renderPackageForAnalysis(analysis: RequestAnalysis, request: string): GeneratedPackageFiles | undefined {
  ...
  if (analysis.family === "lightning_notification") return renderLightningFlash(analysis, request);
...
function renderLightningFlash(analysis: RequestAnalysis, request: string): GeneratedPackageFiles {
  const provide = {
    name: "flash",
