...
export interface ProtocolConversationController {
  applyContinuation?: (
  ...
  const fabric: ProtocolFabric = {
    ...
    async invoke(req: ProtocolInvokeRequest): Promise<ProtocolInvokeResult> {
      ...
      try {
        ...
        options.conversationController?.applyContinuation?.(pi, internalReq.callerNodeId, extractProtocolConversationContinuation(output));
